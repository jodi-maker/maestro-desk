...
export function bhSetEnabled(v) {
...
export function bhSetDayEnabled(idx, v) {
...
export function bhSetDayTime(idx, field, v) {
  const d = BUSINESS_HOURS.days[idx];
  if (!d) return;
...
export function bhRemoveHoliday(date) {
...
export function renderBusinessHours() {
  ...
  const dayRows = BUSINESS_HOURS.days.map((d, i) => `
    <tr>
      ...
      <td>
        <input type="time" class="form-input" value="${window.escAttr(d.start)}" style="max-width:120px" ${admin && d.enabled ? '' : 'disabled'} onchange="bhSetDayTime(${i},'start',this.value)"/>
      </td>
      <td>
        <input type="time" class="form-input" value="${window.escAttr(d.end)}" style="max-width:120px" ${admin && d.enabled ? '' : 'disabled'} onchange="bhSetDayTime(${i},'end',this.value)"/>
      </td>
  ...
  const holidayRows = BUSINESS_HOURS.holidays.map(date => `
    <div style="display:flex;align-items:center;gap:10px;padding:6px 10px;border:1px solid var(--rule);border-radius:var(--r);margin-bottom:4px;background:var(--off2)">
      ...
      ${admin ? `<button class="btn btn-sm" style="margin-left:auto" onclick="bhRemoveHoliday(${window.escHtml(JSON.stringify(date))})">Remove</button>` : ''}
    </div>`).join('');
