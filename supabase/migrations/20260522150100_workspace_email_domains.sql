...
-- RLS — standard workspace_id scope + platform-admin escape hatch baked in
-- from the start (no ALTER needed; this is a new policy).

alter table workspace_email_domains enable row level security;

create policy workspace_email_domains_ws on workspace_email_domains
