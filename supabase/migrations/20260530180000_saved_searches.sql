...
-- RLS: caller sees + writes only their own searches, scoped to a
-- workspace they're in. The user_id = auth.uid() clause is the
-- per-user gate; is_workspace_member covers the workspace scope.
